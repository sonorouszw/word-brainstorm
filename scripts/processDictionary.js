 ...
 * 2. 解析为结构化数据
 * 3. 过滤中文词语（≥2字）
 * 4. 构建倒排索引（汉字 → 包含该字的词ID列表）
 * 5. 输出 dictionary.json 和 charIndex.json 到 frontend/src/data/
 *
 * CC-CEDICT 格式：
...
const DICT_URL = 'https://www.mdbg.net/chinese/export/cedict/cedict_1_0_ts_utf-8_mdbg.txt.gz';
...
/**
 * 解析拼音字符串，移除声调数字，返回干净拼音
 * "chuan2 tong3" → "chuan tong"
 */
...
/**
 * 解析单行 CC-CEDICT 条目
 * 返回 { word, pinyin, definition } 或 null
 */
function parseLine(line) {
  if (!line || line.startsWith('#')) return null;

  // 格式: 傳統 传统 [pinyin] /def1/def2/...
  const match = line.match(/^(\S+)\s+(\S+)\s+\[([^\]]+)\]\s+\/(.+)\/$/);
  if (!match) return null;

  const [, traditional, simplified, pinyinRaw, defRaw] = match;

  // 只保留简体中文词（≥2个字）
  const word = simplified;
  ...
  return {
    word,
    pinyin: cleanPinyin(pinyinRaw),
    definition: defRaw.replace(/\//g, '；'),
  };
}
...
function downloadAndProcess() {
  ...
  https.get(DICT_URL, (res) => {
    ...
    console.log('💡 将使用内置备用词典...');
    generateFallbackDictionary();
  });
}
...
function handleResponse(res) {
  ...
  res.on('end', () => {
    ...
    console.log(`   已下载 ${(buffer.length / 1024).toFixed(0)} KB`);
    ...
    zlib.gunzip(buffer, (err, decompressed) => {
      ...
      console.log(`   解压后 ${(text.length / 1024 / 1024).toFixed(1)} MB`);
      ...
      for (const line of lines) {
        ...
        if (entry) {
          ...
          // 构建倒排索引
          const chars = extractChars(entry.word);
          for (const ch of chars) {
            if (!charIndex[ch]) {
              charIndex[ch] = [];
            }
            charIndex[ch].push(idx);
          }
        }
      }

      console.log(`   解析完成：${dictionary.length} 个词语，${Object.keys(charIndex).length} 个汉字`);
      saveResults(dictionary, charIndex);
    });
  });
}

function saveResults(dictionary, charIndex) {
  // 确保输出目录存在
 ...
 * 当 CC-CEDICT 下载失败时使用
 */
function generateFallbackDictionary() {
  console.log('📝 生成内置备用词典...');

  // 精选 ~300 个常用汉语词语，含拼音和释义
  const words = [
    // 自然
    ...
    { word: '梦想', pinyin: 'meng xiang', definition: '理想；愿望' },
    ...
    { word: '春天', pinyin: 'chun tian', definition: '四季之首' },
    { word: '春雨', pinyin: 'chun yu', definition: '春季下的雨' },
    { word: '风声', pinyin: 'feng sheng', definition: '风的声音' },
    { word: '风景', pinyin: 'feng jing', definition: '自然或人文景观' },
    { word: '风光', pinyin: 'feng guang', definition: '风景；景象' },
    { word: '雪山', pinyin: 'xue shan', definition: '覆盖着积雪的山' },
    { word: '雪地', pinyin: 'xue di', definition: '被雪覆盖的地面' },
    { word: '风霜', pinyin: 'feng shuang', definition: '风和霜；比喻艰苦' },
    ...
    { word: '光彩', pinyin: 'guang cai', definition: '光泽和色彩' },
    ...
    { word: '快乐', pinyin: 'kuai le', definition: '愉快欢乐' },
    ...
    { word: '勇气', pinyin: 'yong qi', definition: '胆量和气魄' },
    { word: '勇敢', pinyin: 'yong gan', definition: '有勇气和胆量' },
    { word: '希望', pinyin: 'xi wang', definition: '对未来的期待' },
    { word: '失望', pinyin: 'shi wang', definition: '希望落空后感到沮丧' },
    { word: '盼望', pinyin: 'pan wang', definition: '殷切地期望' },
    { word: '梦想', pinyin: 'meng xiang', definition: '理想；愿望' },
    { word: '梦中', pinyin: 'meng zhong', definition: '在梦境中' },
    { word: '梦幻', pinyin: 'meng huan', definition: '如梦般奇幻' },
    { word: '热情', pinyin: 're qing', definition: '热烈的情感' },
    { word: '无情', pinyin: 'wu qing', definition: '没有感情' },
    { word: '友情', pinyin: 'you qing', definition: '朋友之间的情谊' },
    { word: '温情', pinyin: 'wen qing', definition: '温柔的情感' },
    { word: '温柔', pinyin: 'wen rou', definition: '温和柔顺' },
    { word: '柔和', pinyin: 'rou he', definition: '温和不强烈' },
    { word: '温和', pinyin: 'wen he', definition: '不严厉不粗暴' },
    { word: '生气', pinyin: 'sheng qi', definition: '因不满而不高兴' },
    { word: '愤怒', pinyin: 'fen nu', definition: '因不满而激愤' },
    { word: '恐惧', pinyin: 'kong ju', definition: '害怕；恐慌' },
    ...
    { word: '惊喜', pinyin: 'jing xi', definition: '意外的喜悦' },
    ...
    { word: '老师', pinyin: 'lao shi', definition: '教师' },
    { word: '老人', pinyin: 'lao ren', definition: '老年人' },
    { word: '童年', pinyin: 'tong nian', definition: '儿童时代' },
    { word: '少年', pinyin: 'shao nian', definition: '十岁到十五六岁的阶段' },
    { word: '青年', pinyin: 'qing nian', definition: '十几到二十几岁的年龄段' },
    { word: '英雄', pinyin: 'ying xiong', definition: '有杰出贡献的人' },
    { word: '英勇', pinyin: 'ying yong', definition: '非常勇敢' },
    { word: '诗人', pinyin: 'shi ren', definition: '写诗的人' },
    { word: '诗歌', pinyin: 'shi ge', definition: '诗的总称' },
    { word: '诗人', pinyin: 'shi ren', definition: '写诗的创作者' },
    { word: '画家', pinyin: 'hua jia', definition: '从事绘画的艺术家' },
    { word: '画廊', pinyin: 'hua lang', definition: '展示画作的场所' },
    { word: '歌手', pinyin: 'ge shou', definition: '唱歌的人' },
    { word: '歌声', pinyin: 'ge sheng', definition: '唱歌的声音' },
    { word: '歌曲', pinyin: 'ge qu', definition: '供人歌唱的作品' },
    { word: '作家', pinyin: 'zuo jia', definition: '从事文学创作的人' },
    { word: '作品', pinyin: 'zuo pin', definition: '创作的成品' },
    { word: '母亲', pinyin: 'mu qin', definition: '妈妈' },
    { word: '父亲', pinyin: 'fu qin', definition: '爸爸' },
    ...
    { word: '时间', pinyin: 'shi jian', definition: '物质运动持续的属性' },
    { word: '时空', pinyin: 'shi kong', definition: '时间和空间' },
    { word: '生命', pinyin: 'sheng ming', definition: '生物具有的活动能力' },
    { word: '生活', pinyin: 'sheng huo', definition: '人或生物为了生存和发展而进行的各种活动' },
    { word: '生日', pinyin: 'sheng ri', definition: '人出生的日子' },
    { word: '自由', pinyin: 'zi you', definition: '不受限制的状态' },
    { word: '自然', pinyin: 'zi ran', definition: '自然界；不勉强' },
    { word: '自在', pinyin: 'zi zai', definition: '自由舒适' },
    { word: '命运', pinyin: 'ming yun', definition: '人生际遇' },
    { word: '运气', pinyin: 'yun qi', definition: '幸运或不幸的遭遇' },
    { word: '记忆', pinyin: 'ji yi', definition: '对过去经验的保留' },
    { word: '回忆', pinyin: 'hui yi', definition: '回想过去的事' },
    { word: '记住', pinyin: 'ji zhu', definition: '记在心里不忘记' },
    { word: '未来', pinyin: 'wei lai', definition: '尚未到来的时间' },
    { word: '将来', pinyin: 'jiang lai', definition: '未来；以后' },
    { word: '世界', pinyin: 'shi jie', definition: '全球所有地方' },
    { word: '人间', pinyin: 'ren jian', definition: '人类社会' },
    { word: '历史', pinyin: 'li shi', definition: '过去的事件' },
    { word: '经历', pinyin: 'jing li', definition: '亲身遇到过的事' },
    { word: '文化', pinyin: 'wen hua', definition: '精神财富总和' },
    { word: '文明', pinyin: 'wen ming', definition: '社会发展阶段' },
    { word: '文艺', pinyin: 'wen yi', definition: '文学和艺术' },
    { word: '艺术', pinyin: 'yi shu', definition: '创造性活动' },
    { word: '科学', pinyin: 'ke xue', definition: '系统化的知识' },
    { word: '科技', pinyin: 'ke ji', definition: '科学技术' },
    { word: '真理', pinyin: 'zhen li', definition: '正确的认识' },
    { word: '真相', pinyin: 'zhen xiang', definition: '事物的真实情况' },
    { word: '智慧', pinyin: 'zhi hui', definition: '洞察和解决问题的能力' },
    { word: '智力', pinyin: 'zhi li', definition: '认识理解事物的能力' },
    { word: '灵魂', pinyin: 'ling hun', definition: '精神内核' },
    { word: '心灵', pinyin: 'xin ling', definition: '内心精神世界' },
    { word: '永恒', pinyin: 'yong heng', definition: '永远不变' },
    { word: '永久', pinyin: 'yong jiu', definition: '长久持续' },
    { word: '和平', pinyin: 'he ping', definition: '无战争状态' },
    { word: '和睦', pinyin: 'he mu', definition: '相处融洽' },
    { word: '战争', pinyin: 'zhan zheng', definition: '武装冲突' },
    { word: '战场', pinyin: 'zhan chang', definition: '交战的地方' },
    { word: '力量', pinyin: 'li liang', definition: '力气；能力' },
    { word: '力气', pinyin: 'li qi', definition: '体力' },
    { word: '旅行', pinyin: 'lv xing', definition: '出外游览' },
    { word: '旅游', pinyin: 'lv you', definition: '旅行游览' },
    { word: '学习', pinyin: 'xue xi', definition: '获取知识' },
    { word: '学生', pinyin: 'xue sheng', definition: '在校学习的人' },
    { word: '思考', pinyin: 'si kao', definition: '深入思维' },
    { word: '思想', pinyin: 'si xiang', definition: '理性认识；念头' },
    { word: '创造', pinyin: 'chuang zao', definition: '做新事物' },
    { word: '创新', pinyin: 'chuang xin', definition: '创造新的' },
    { word: '探索', pinyin: 'tan suo', definition: '多方寻求' },
    { word: '探究', pinyin: 'tan jiu', definition: '探索追究' },
    { word: '飞翔', pinyin: 'fei xiang', definition: '在空中飞行' },
    { word: '飞行', pinyin: 'fei xing', definition: '在空中的移动' },
    { word: '奔跑', pinyin: 'ben pao', definition: '快速跑步' },
    { word: '奔驰', pinyin: 'ben chi', definition: '快速奔跑' },
    { word: '歌唱', pinyin: 'ge chang', definition: '唱歌抒发情感' },
    { word: '歌舞', pinyin: 'ge wu', definition: '唱歌跳舞' },
    { word: '舞蹈', pinyin: 'wu dao', definition: '用身体动作表演' },
    { word: '舞台', pinyin: 'wu tai', definition: '表演的台子' },
    { word: '写作', pinyin: 'xie zuo', definition: '文字创作' },
    { word: '书写', pinyin: 'shu xie', definition: '写字' },
    { word: '绘画', pinyin: 'hui hua', definition: '用画笔创作' },
    { word: '画家', pinyin: 'hua jia', definition: '绘画的人' },
    { word: '阅读', pinyin: 'yue du', definition: '看书或文字' },
    { word: '读者', pinyin: 'du zhe', definition: '阅读的人' },
    { word: '倾听', pinyin: 'qing ting', definition: '仔细听' },
    { word: '聆听', pinyin: 'ling ting', definition: '恭敬地听' },
    { word: '拥抱', pinyin: 'yong bao', definition: '用双臂环抱' },
    { word: '拥有', pinyin: 'yong you', definition: '具有；领有' },
    { word: '追逐', pinyin: 'zhui zhu', definition: '追赶追求' },
    { word: '追求', pinyin: 'zhui qiu', definition: '努力争取' },
    { word: '城市', pinyin: 'cheng shi', definition: '大型居住区' },
    { word: '都市', pinyin: 'du shi', definition: '大城市' },
    { word: '乡村', pinyin: 'xiang cun', definition: '农村' },
    { word: '家乡', pinyin: 'jia xiang', definition: '故乡' },
    { word: '书店', pinyin: 'shu dian', definition: '卖书的店' },
    { word: '书法', pinyin: 'shu fa', definition: '汉字的书写艺术' },
    { word: '海边', pinyin: 'hai bian', definition: '海边缘' },
    { word: '海岸', pinyin: 'hai an', definition: '陆地靠海的部分' },
    { word: '山顶', pinyin: 'shan ding', definition: '山顶端' },
    { word: '山谷', pinyin: 'shan gu', definition: '山间的谷地' },
    { word: '市场', pinyin: 'shi chang', definition: '交易场所' },
    { word: '广场', pinyin: 'guang chang', definition: '宽阔的公共空间' },
    { word: '宇宙', pinyin: 'yu zhou', definition: '包含万物的空间' },
    { word: '太空', pinyin: 'tai kong', definition: '外层空间' },
    { word: '天堂', pinyin: 'tian tang', definition: '极乐世界' },
    { word: '天气', pinyin: 'tian qi', definition: '气象变化' },
    { word: '故乡', pinyin: 'gu xiang', definition: '出生或成长地' },
    { word: '故人', pinyin: 'gu ren', definition: '老朋友' },
    { word: '远方', pinyin: 'yuan fang', definition: '遥远的地方' },
    { word: '远离', pinyin: 'yuan li', definition: '远远离开' },
    { word: '书籍', pinyin: 'shu ji', definition: '装订的著作' },
    { word: '书本', pinyin: 'shu ben', definition: '书籍的统称' },
    { word: '音乐', pinyin: 'yin yue', definition: '乐音艺术' },
    { word: '声音', pinyin: 'sheng yin', definition: '听觉感受' },
    { word: '电影', pinyin: 'dian ying', definition: '影像故事' },
    { word: '电子', pinyin: 'dian zi', definition: '与电子技术相关的' },
    { word: '照片', pinyin: 'zhao pian', definition: '拍摄的图片' },
    { word: '照片', pinyin: 'zhao pian', definition: '用相机拍的图片' },
    { word: '镜子', pinyin: 'jing zi', definition: '反射影像的物体' },
    { word: '镜头', pinyin: 'jing tou', definition: '拍摄的光学部件' },
    { word: '灯火', pinyin: 'deng huo', definition: '灯光；照明' },
    { word: '灯光', pinyin: 'deng guang', definition: '灯发出的光' },
    { word: '颜色', pinyin: 'yan se', definition: '视觉色彩感受' },
    { word: '色彩', pinyin: 'se cai', definition: '各种颜色' },
    { word: '味道', pinyin: 'wei dao', definition: '味觉感受' },
    { word: '气味', pinyin: 'qi wei', definition: '嗅觉感受' },
    { word: '故事', pinyin: 'gu shi', definition: '讲述的事件' },
    { word: '事故', pinyin: 'shi gu', definition: '意外事件' },
    { word: '礼物', pinyin: 'li wu', definition: '赠送的物品' },
    { word: '礼仪', pinyin: 'li yi', definition: '礼节和仪式' },
    { word: '秘密', pinyin: 'mi mi', definition: '不为人知的事' },
    { word: '神秘', pinyin: 'shen mi', definition: '不可测的' },
    { word: '宝藏', pinyin: 'bao zang', definition: '财宝' },
    { word: '宝石', pinyin: 'bao shi', definition: '珍贵的矿物' },
    { word: '光芒', pinyin: 'guang mang', definition: '亮光' },
    { word: '光线', pinyin: 'guang xian', definition: '传播方向的光束' },
    { word: '影子', pinyin: 'ying zi', definition: '暗影' },
    { word: '影响', pinyin: 'ying xiang', definition: '对他物的作用' },
  ];

  // 去重
  const seen = new Set();
  const dictionary = words.filter(w => {
  ...
  console.log(`   生成了 ${dictionary.length} 个词语，${Object.keys(charIndex).length} 个汉字索引`);
  saveResults(dictionary, charIndex);
}
